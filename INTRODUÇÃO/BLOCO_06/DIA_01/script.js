...
function defaultValidation(input,name) {
  ...
  if (validation.required && trimeed.length === 0){
    return false;
  }
...
function dateValidations(input, name){
...
function getSelectedOpotion(select){
  return select.options[select.selectedIndex];
}

function selectValidation (select, name) {
  let option = getSelectedOpotion(select);
  let validation = inputs[name];

  if(validation.required && (!option.disabled)) {
    return false;
  }
  return true;
}
